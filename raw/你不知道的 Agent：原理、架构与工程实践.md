...
tags:
  - "clippings"
---
<svg width="100%" height="200" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" role="img" aria-label="你不知道的 Agent：原理、架构与工程实践"><path id="svg-title-path"></path><text font-size="36" font-weight="600" fill="#ffffff" x="50%" y="50%" text-anchor="middle" word-spacing="2.0" letter-spacing="1.2"><textPath xlink:href="#svg-title-path">你不知道的 Agent：原理、架构与工程实践</textPath></text></svg>

#### Categories: Share

![Agent 架构封面图](https://gw.alipayobjects.com/zos/k/37/agentr.png?x-oss-process=image/auto-orient,1/resize,w_2000/format,webp)
...
```typescript
TYPESCRIPTconst messages: MessageParam[] = [{ role: "user", content: userInput }];
...
```markdown
MARKDOWN### Compact Instructions 如何保留关键信息
...
```typescript
TYPESCRIPT// 差：参数模糊，出错只返回字符串，Agent 不知道怎么修正
const tool = {
...
```typescript
TYPESCRIPTconst updateTool = betaZodTool({
  name: "update_yuque_post",
...
```js
JS// 子 Agent 有独立的 messages[]，跑完只回传摘要
...
```js
JS// 消息结构：结构化，有状态，append-only，崩溃可恢复
{
...
```js
JS// MessageBus：渠道和 Agent 之间的解耦层
class MessageBus {
...
```markdown
MARKDOWN# SOUL.md，定义 Agent 的身份、约束和完成标准
...
```typescript
TYPESCRIPTconst AUTHORIZED_USERS = new Set(["user_id_tang", "user_id_other"]);
...
```typescript
TYPESCRIPTconst WORKSPACE = path.resolve("/Users/tang/workspace");
...
```typescript
TYPESCRIPTasync function auditedShell(args: string[], userId: string): Promise<string> {
  // 执行前记录：时间、用户、命令
  await fs.appendFile(
...
```typescript
TYPESCRIPTfunction wrapUntrustedContent(source: string, content: string): string {
  return [
...
```typescript
TYPESCRIPTconst providers = ["Anthropic", "OpenAI", "Anthropic Sonnet"];
