...
tags:
  - "clippings"
---
<svg width="100%" height="200" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" role="img" aria-label="你不知道的 Claude Code：架构、治理与工程实践"><path id="svg-title-path"></path><text font-size="36" font-weight="600" fill="#ffffff" x="50%" y="50%" text-anchor="middle" word-spacing="2.0" letter-spacing="1.2"><textPath xlink:href="#svg-title-path">你不知道的 Claude Code：架构、治理与工程实践</textPath></text></svg>
...
```
PLAINTEXT收集上下文 → 采取行动 → 验证结果 → [完成 or 回到收集]
     ↑                    ↓
...
![](https://cdn.fliggy.com/upic/g1Ce7x.png?x-oss-process=image/auto-orient,1/resize,w_2000/format,webp)
...
```
PLAINTEXT始终常驻    → CLAUDE.md：项目契约 / 构建命令 / 禁止事项
...
![/context 命令输出，可以看到各来源的 token 占比](https://cdn.fliggy.com/upic/DOPgjN.png?x-oss-process=image/auto-orient,1/resize,w_2000/format,webp)
...
![](https://cdn.fliggy.com/upic/6qvOIC.png?x-oss-process=image/auto-orient,1/resize,w_2000/format,webp)
...
![](https://cdn.fliggy.com/upic/iOfkNM.png?x-oss-process=image/auto-orient,1/resize,w_2000/format,webp)
...
![Finding the sweet spot](https://cdn.fliggy.com/upic/aMfLwM.png?x-oss-process=image/auto-orient,1/resize,w_2000/format,webp)
...
![Improving Elicitation & the AskUserQuestion tool](https://cdn.fliggy.com/upic/JHqSz5.png?x-oss-process=image/auto-orient,1/resize,w_2000/format,webp)
...
![Updating with Capabilities - Tasks & Todos](https://cdn.fliggy.com/upic/hXrzJK.png?x-oss-process=image/auto-orient,1/resize,w_2000/format,webp)
...
![Hooks 配置界面](https://cdn.fliggy.com/upic/5J83L4.png?x-oss-process=image/auto-orient,1/resize,w_2000/format,webp)
...
![Hooks 在执行过程中的介入点](https://cdn.fliggy.com/upic/O9aa1Q.png?x-oss-process=image/auto-orient,1/resize,w_2000/format,webp)
...
![Lay Out Your Prompt for Caching](https://cdn.fliggy.com/upic/tCDytz.png?x-oss-process=image/auto-orient,1/resize,w_2000/format,webp)

Prompt 缓存是按 **前缀匹配** 工作的，从请求开头到每个 `cache_control` 断点之前的内容都会被缓存。所以这里的顺序很重要：

```
PLAINTEXTClaude Code 的 Prompt 顺序：
...
```bash
BASH/context   # 查看 token 占用结构，排查 MCP 和文件读取占比
...
![/mcp 连接状态，可以看到各 server 的工具数量和 token 消耗](https://cdn.fliggy.com/upic/XwDs5n.png?x-oss-process=image/auto-orient,1/resize,w_2000/format,webp)

```bash
BASH/mcp       # 管理 MCP 连接，检查 token 成本，断开闲置 server
...
```bash
BASHclaude --continue               # 恢复当前目录最近会话，隔天接着做
...
![Keep it simple](https://cdn.fliggy.com/upic/6IysXR.jpg?x-oss-process=image/auto-orient,1/resize,w_2000/format,webp)
...
```bash
BASHclaude plugin marketplace add tw93/waza
